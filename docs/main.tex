...
%-----------------------------------------------
	...
	\begin{enumerate}
		...
		\item On définit la matrice $H=(\tilde{h}_{i,j})_{(i,j)\in\llbracket 1;n\rrbracket^2}\in\mathcal{M}(\mathbb{N})$ tel que pour tout $(i,j)\in\llbracket 1;n\rrbracket^2, \tilde{h}_{i,j}=\left\{\begin{array}{ll}
			...
			+\infty & \text{sinon}
		\end{array}\right.$.
		\item On note $p=(x_p,y_p)$ la case parent. On applique la fonction $\tilde{\delta}$ sur $p$ pour obtenir l'ensemble $E$ des cases atteignables depuis celle-ci. \textbf{DEFINIR DELTA TILDE}
